...
/* ===========================
   MODELOS (DB QUERIES)
   =========================== */
async function createImage(imagen, nombre) {
  try{
      const query = (
        `INSERT INTO X_IMAGEN (imagen, nombre) 
        VALUES ($1, $2) 
        RETURNING *`
      );
      const values = [imagen, nombre]
...
// UPDATE
async function updateImage(id, imagen, nombre) {
  const result = await pool.query(
    `UPDATE x_imagen SET imagen = $1, nombre = $2 WHERE id_imagen = $3 RETURNING *`,
    [imagen, nombre, id]
  );
  return result.rows[0];
}
...
const crearImagen = async (req, res) => {
  ...
  try {
    ...
    // Obtener el nombre del body, si no viene usar un valor por defecto
    const { nombre = 'Imagen sin nombre' } = req.body;

    // Guardar en base de datos
    const nuevaImagen = await createImage(imagePath, nombre);
...
const actualizarImagen = async (req, res) => {
  ...
  try {
    ...
    const updatedImage = await updateImage(id, newImagePath, nombre);
